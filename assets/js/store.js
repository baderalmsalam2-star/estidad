...
const empty = () => ({
  ...
  reminderAt: null,
  /*
   * اسمُ الطالبِ — يُكتَب في ترويسةِ الرئيسيةِ ولا يخرج من الجهاز.
   *
   * وهو **اختياريٌّ كلَّه**: يُتخطّى في أوّلِ شاشةٍ ويبقى التطبيقُ تامّاً، وهو
   * الحالُ الأصليّة (`null` ⇐ «أهلاً بك»).
   *
   * ولا يُرسَل في `sync` (الطابورُ رقمُ سؤالٍ ودرجةٌ ومسارٌ لا غير)، ولا
   * يُكتَب في صورةِ النتيجةِ التي يُشارِكها الطالبُ في واتساب — فتلك تخرج إلى
   * غيره. وهو في ملفِّ النسخةِ الاحتياطيّةِ لأنّ الملفَّ نسخةُ حالِه كلِّها
   * ويبقى على قرصه.
   *
   * وبدخوله صار في التطبيقِ ما يُعَدُّ بياناً شخصيّاً، فبُدِّل نصُّ «حسابي»
   * الذي كان يقول «ولا بياناتٍ شخصية» — لا يُقال ما ليس كذلك.
   */
  name: null,
  /*
 ...
 */
/**
 * تطهيرُ الاسمِ المكتوب.
 *
 * وهو مكتوبٌ بيدٍ ويُعرَض في عنوانِ الشاشةِ (`h1`)، فثلاثةُ قيودٍ عليه:
 * تُزال الأسطرُ والمحارفُ الخفيّةُ (ومنها علاماتُ الاتّجاهِ التي تقلب ما
 * بعدها)، وتُوحَّد الفراغات، ويُقصَر إلى اثنَين وثلاثين محرفاً — فالعنوانُ
 * ٣٦ بكسلاً بخطِّ أميري، وما جاوز ذلك يكسر الترويسةَ على عرضِ الهاتف.
 *
 * ولا يُمسُّ حرفُه ولا تُعرَّب أرقامُه: هو اسمُ صاحبِه كما كتبه.
 */
const cleanName = (v) => String(v ?? '')
  // ما يقطع سطراً يصير فراغاً — ولا يُحذَف، وإلا التصقَ ما حولَه: «أحمد⏎بن
  // علي» كانت تصير «أحمدبن علي».
  .replace(/[\u0000-\u001f\u007f\u2028\u2029]/g, ' ')
  // وما لا عرضَ له يُحذَف: علاماتُ الاتّجاهِ تقلب ما بعدها في سطرٍ عربيّ.
  .replace(/[\u200b-\u200f\u202a-\u202e\u2066-\u2069]/g, '')
  .replace(/\s+/g, ' ')
  .trim()
  .slice(0, 32)
...
function sane(raw) {
  ...
  // الاسمُ من يدِ الطالب، فيُقلَّم ويُقصَر ويُجرَّد ممّا يكسر سطراً.
  if (typeof raw.name === 'string') d.name = cleanName(raw.name);
...
/** اسمُ الطالبِ إن كتبه — و`null` إن لم يكتبه أو محاه. */
export const name = () => cache.name;

export function setName(v) {
  cache.name = cleanName(v);
  write();
  return cache.name;
}
 ...
 * طويلة. والسلسلةُ تُقرَأ من سجلِّ `days`، وبيانُ ذلك عند `days` في `empty`.
 *
 * وكان مكتوباً ههنا «مُشتَقّةٌ من طوابع `answers[].at` — لا سجلَّ زائداً يُحفَظ»،
 * وهو الوصفُ الذي يقول تعليقُ `days` نفسُه إنّه كان **الغلط**: الاشتقاقُ من
 * `answers[].at` يعاقب الطالبَ على المواظبة. فبقيت الجملةُ في موضعها الثاني
 * تصفُ كوداً لم يعد قائماً (`countsByDay` تقرأ `cache.days`، و`record` تكتب فيه).
 */
const dayKey = (t) => {
  const d = new Date(t);
