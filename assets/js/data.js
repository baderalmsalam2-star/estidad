...
/**
 * من أين جاء كلُّ سؤالٍ — تُحسَب من البنكِ لحظةَ السؤالِ لا تُكتَب يداً.
 *
 * وهذه هي الأرقامُ التي يُبنى عليها نصُّ التحذير. ولا تُكتَب في الشاشاتِ رقماً
 * لأنّها تتبدّل مع كلِّ مقابلةٍ يُجريها صاحبُ التطبيق — ورقمٌ مكتوبٌ يداً في
 * تحذيرٍ يتقادم، فيصير التحذيرُ نفسُه كذباً بعد شهر.
 *
 *   • `onPage`     — قوبِل على **صورةِ صفحةٍ مطبوعة**، وهو أقواها.
 *   • `ocr`        — قوبِل على نصٍّ مقروءٍ آلياً (OCR)، وهو أضعفُ: قد يُخطئ
 *                    القارئُ الآليُّ في الحرفِ فيُقابَل على نصٍّ ليس نصَّ الكتاب.
 *   • `uncollated` — لم يُقابَل على شيءٍ بعد.
 */
export function provenance() {
  const out = { onPage: 0, ocr: 0, uncollated: 0, total: state.questions.length };
  for (const q of state.questions) {
    if (q.provenance === 'generated-from-page' || q.provenance === 'collated-on-page') out.onPage += 1;
    else if (q.provenance === 'generated-from-ocr-text') out.ocr += 1;
    else out.uncollated += 1;
  }
  return out;
}

/**
 * نصُّ التحذيرِ — موضعٌ واحدٌ يقرؤه الطالبُ في شاشتَين.
 *
 * ── لِمَ بُدِّل ───────────────────────────────────────────────────────────
 *
 * كان يقول: «الأسئلةُ اجتهادٌ تدريبيٌّ مبنيٌّ على الكتب المقرَّرة، لا أسئلةَ
 * اختباراتٍ رسمية». وهو صادقٌ فيما نفى، ساكتٌ عمّا هو أهمُّ منه: أنّ الأسئلةَ
 * والأجوبةَ **استُخرِجت من الكتبِ آلياً**، وأنّ ما قوبِل منها على صورةِ صفحةٍ
 * ليس كلَّها، وأنّه **لم يقرأها عالِمٌ بعد**.
 *
 * و«اجتهادٌ تدريبيّ» تُفهَم من إمامٍ في مسجدٍ اجتهادَ إنسانٍ نظرَ في الكتب،
 * لا استخراجَ آلةٍ لم يُراجِعها أحد. والفرقُ بينهما هو كلُّ ما يحتاج أن
 * يعرفه قبل أن يبنيَ عليها.
 *
 * والعددُ يُقال لأنّه يُفرِّق: من رأى «٣١٤٢ قوبِل على صورةِ صفحةٍ» علِم أنّ
 * وراءَ هذا عملاً، وعلِم مع ذلك أنّ بقيّتَه لم يُقابَل بعدُ فيتحرّز فيه.
 */
export function disclaimer() {
  const p = provenance();
  return 'الأسئلةُ والأجوبةُ استُخرِجت من الكتب المقرَّرة آلياً: '
    + `${arNum(p.onPage)} منها قوبِل على صورةِ صفحتِه في الكتاب، `
    + `و${arNum(p.ocr)} على نصٍّ مقروءٍ آلياً، و${arNum(p.uncollated)} لم يُقابَل بعد. `
...
/**
 * أسئلةٌ لا تُفهَم وحدَها — ضميرٌ لا مرجعَ له، أو بدءٌ في وسطِ الكلام.
 *
 * ── من أين جاءت ─────────────────────────────────────────────────────────
 *
 * البنكُ مُولَّدٌ من جُمَلِ الكتابِ على ترتيبها، والكتابُ يَسوق المسألةَ ثمّ
 * يَعطِف عليها بضميرٍ يعود إلى ما قبلَه: «وإذا تمَّ العقدُ وجبَ على المرأةِ
 * أن تُسلِّمَ نفسَها… ولا يجبُ عليها التسليمُ إنْ طلبَها وهي مُحرِمةٌ». فإذا
 * وُلِّد من الجملةِ الثانيةِ سؤالٌ خرجَ: «متى لا يجبُ عَلَيها التسليمُ؟» —
 * و«ها» تعود إلى امرأةٍ لم تُذكَر فيه.
 *
 * ويُقرَأ في الكتابِ سليماً لأنّ ما قبلَه حاضر، ويُقرَأ في الاختبارِ مُعمًّى
 * لأنّ السؤالَ يُسحَب وحدَه. وأوّلُ من أبلغَ عن هذا أبلغَ عنه:
 * «هذا السؤال غير واضح لاستعمال الضمير؟ التصريح أوضح» — وهو محقّ.
 *
 * ── وهذه مُرشَّحاتٌ لا أحكام ────────────────────────────────────────────
 *
 * التمييزُ بين ضميرٍ مرجعُه في السؤالِ وضميرٍ مرجعُه خارجَه لا يُقطَع به
 * بقاعدةٍ نصّيّة، وكلُّ قاعدةٍ جرّبتُها أخطأت في الجهتَين. فتُجمَع بعلامتَين
 * ظاهرتَين، ويُعرَض ما اجتمع فيهما على صاحبِ التطبيقِ ليحكم — ولا يُعدَّل
 * سؤالٌ من نفسِه.
 *
 *   • أن يبدأ السؤالُ بحرفِ عطفٍ أو شرطٍ (و… إنْ… إذا… لو… ثمّ…) — فهو
 *     مبدوءٌ في وسطِ الكلامِ على كلِّ حال.
 *   • أو أن يكون قصيراً وفيه ضميرٌ متّصلٌ ولا اسمَ ظاهراً قبلَه يصلح مرجعاً.
 */
export function unclear() {
  /*
   * ── لماذا كلماتٌ لا `\b` ────────────────────────────────────────────────
   *
   * جُرِّبت بـ`\b` أوّلاً فأعطت **صفراً** في المتصفّحِ وسبعةً وستّين في بايثون.
   * والعلّةُ أنّ `\b` في JavaScript محدودةٌ بـ`[A-Za-z0-9_]`: فالحرفُ العربيُّ
   * ليس عندها «حرفَ كلمة»، فـ`\bعليها\b` لا تُطابِق شيئاً أبداً. وبايثون
   * تعدُّه حرفَ كلمةٍ فتُطابِق — ففحصٌ في بايثون يُصدِّق قاعدةً لا تعمل في
   * التطبيق.
   *
   * والحلُّ ليس `(?<!...)`: النظرُ إلى الخلفِ لا تعرفه سفاري قبل ١٦٫٤،
   * وأرضيّةُ التطبيقِ iOS ١٤٫٥. فيُقطَّع النصُّ كلماتٍ وتُوازَن الكلمةُ كلُّها
   * — وهو أوضحُ من كلِّ حيلةٍ في التعبيرِ النمطيِّ على كلِّ حال.
   */
  const bare = (t) => String(t)
    .replace(/[ً-ْٰـ]/g, '')          // تُجرَّد الحركات
    .replace(/[؟،.:!ـ«»﴿﴾()"'؛]/g, ' ');            // ويُفصَل ما يلتصق

  /** ضميرٌ متّصلٌ بحرفِ جرٍّ — أظهرُ صورِ الضميرِ الذي يتعلّق بما قبلَ السؤال. */
  const PREP = new Set(['عليها', 'عليه', 'عليهم', 'لها', 'له', 'لهم', 'بها', 'به',
    'بهم', 'منها', 'منه', 'فيها', 'فيه', 'إليها', 'إليه', 'عنها', 'عنه']);
  /** أو فعلٌ لَحِقَه ضميرُ نصبٍ: «ألقاها»، «قتلَه»، «وهبَه». */
  const bound = (w) => w.length > 3 && (w.endsWith('ها') || w.endsWith('هم') || w.endsWith('ه'));
  /** صدرٌ يدلُّ على أنّ الكلامَ مُتَّصلٌ بما قبلَه. */
  const STARTS = new Set(['إن', 'وإن', 'إذا', 'وإذا', 'لو', 'ولو', 'فإن', 'ثم']);
  /** اسمٌ ظاهرٌ يصلح مرجعاً: معرَّفٌ بأل، أو مؤنَّثٌ بالتاء، أو اسمٌ موصول. */
  const noun = (w) => /^ال[ء-ي]{2,}/.test(w) || /[ء-ي]{3,}ة$/.test(w)
    || w === 'من' || w === 'ما' || w === 'مَن';

  const out = [];
  for (const q of state.questions) {
    const words = bare(q.question || '').split(/\s+/).filter(Boolean);
    let at = -1;
    for (let i = 0; i < words.length; i += 1) {
      if (PREP.has(words[i]) || bound(words[i])) { at = i; break; }
    }
    if (at < 0) continue;
    if (words.slice(0, at).some(noun)) continue;     // مرجعُه في السؤال
    const mid = STARTS.has(words[0]);
    // الطويلُ الغالبُ أنّ مرجعَه فيه وإن لم تلتقطه القاعدة، فلا يُتَّهم.
    if (!mid && words.length > 8) continue;
    out.push({ q, why: mid ? 'يبدأ في وسطِ الكلام' : 'قصيرٌ وضميرُه بلا مرجع' });
  }
  return out;
}
...
export function flashcardsOf(track) {
  /*
   * بلا `\b` — فهي في JavaScript لا تعرف الحرفَ العربيَّ حرفَ كلمة.
   *
   * كانت `/\b(اثنان|ثلاثة|…)\b/` فلم تُطابِق شيئاً **قطّ**: `\b` تشترط انتقالاً
   * بين `[A-Za-z0-9_]` وغيرِه، والفراغُ والتاءُ كلاهما «غيرُ حرفٍ» عندها فلا
   * حدَّ بينهما. فكان التقاطُ لفظِ العددِ في البطاقاتِ غيرِ المُهيكَلةِ ميتاً
   * من يومِ كُتِب، ولا شيءَ يُخفِق — إنّما تخرج البطاقةُ بلا عدد.
   *
   * ووُجِدت بفحصِ المشروعِ كلِّه بعد أن أوقعتني `\b` نفسُها في `unclear()`.
   * وحدُّ الكلمةِ ههنا غيرُ محتاجٍ إليه: الألفاظُ مُميَّزةٌ لا تقع جزءاً من
   * كلمةٍ أخرى.
   */
  const NUMBER_WORDS = /(اثنان|ثلاثة|أربعة|خمسة|ستة|سبعة|ثمانية|تسعة|عشرة|أحد عشر|خمسٌ|سبعٌ|تسعٌ|ثلاثٌ)/;
