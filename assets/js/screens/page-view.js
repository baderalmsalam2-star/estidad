...
import { el, ar, empty, dropOverlayEntry } from '../ui.js';
 ...
 *
 * فمن أغلقَ بيدِه بُدِّل معنى قيدِه (`dropOverlayEntry`) فصار قيدَ رجوعٍ عن
 * الشاشة. ومن أغلقَ بزرِّ الرجوعِ فالقيدُ مستهلَكٌ أصلاً فلا يُمَسّ — فيُفرَّق
 * بينهما بـ`fromPop`.
 */
...
export function closePage(fromPop = false) {
  ...
  // القيدُ اليتيمُ يُبدَّل معناه فيصير قيدَ رجوعٍ عن الشاشة — لا يُستهلَك
  // بـ`history.back()`، فذاك غيرُ متزامنٍ ويُعيد الرسمَ. (انظر
  // `dropOverlayEntry` في ui.js.)
  if (had && !fromPop) dropOverlayEntry();
