    ...
    el('div.card', { style: { gap: '8px' } }, [
      ...
      el('button', {
        onclick: async () => {
          ...
          /**
           * ويُقال ما وقعَ لا ما طُلِب.
           *
           * `audio.wipe()` تُرجِع `false` عند `onerror` وعند `onblocked`،
           * و`onblocked` حالٌ واقعيّةٌ بحتة: لسانانِ من التطبيقِ مفتوحانِ في
           * المتصفّحِ نفسِه يحجُبانِ الحذفَ — وقد كُتِب ذلك عند `audio.js:84`
           * بيدِ المشروعِ نفسِه، ثمّ لم تُقرأ القيمةُ المُرجَعة. فكان يُقال
           * «مُحيت تسجيلاتك.» وصوتُه باقٍ على جهازه.
           *
           * وهذا أخصُّ ما في التطبيق: صوتُ الطالبِ وهو يقرأ القرآن.
           */
          const wiped = await audio.wipe();
          alert(wiped
            ? 'مُحيت تسجيلاتك.'
            : 'تعذّر المحو — قد يكون التطبيقُ مفتوحاً في لسانٍ آخَر. '
              + 'أغلِقْ ألسنتَه الأخرى ثمّ أعِدِ المحاولة.');
        },
        style: {
      ...
      el('button', {
        onclick: async () => {
          ...
          // و`store.reset()` تُرجِع ما أرجعته `audio.wipe()`: فإن حُجِبت
          // القاعدةُ مُحِي التقدُّمُ وبقي الصوت — ويُقال ذلك بعينه، فالوعدُ
          // في نصِّ التأكيدِ كان محوَهما معاً.
          const wiped = await store.reset();
          if (!wiped) {
            alert('مُحي تقدّمك، وتعذّر محوُ تسجيلات التسميع — قد يكون التطبيقُ '
              + 'مفتوحاً في لسانٍ آخَر. أغلِقْ ألسنتَه الأخرى ثمّ امسحْها من «حسابي».');
          }
          go('track');
        },
        style: {
  ...
  const draw = () => {
    ...
    card.replaceChildren(
      ...
      waiting
        ...
        : null,
      // وما سقط من الطابورِ لبلوغِه سقفَه يُقال، ولا يُكتَم: الطالبُ يرى
      // العددَ ينقص فيحسبه وصل، وهو لم يصل ولن يصل.
      sync.dropped()
        ? el('span.fine', { style: { textAlign: 'start', color: 'var(--ink-5)' } },
            `و${ar(sync.dropped())} إجابةً سقطت من الطابور لطولِ انقطاعِ الشبكة، فلم تصل.`)
        : null,
    );
  };
...
function reminderCard() {
  ...
  const draw = () => {
    ...
    card.replaceChildren(
      ...
      // والحدثُ يخرج من التطبيقِ إلى تقويمِ الجهاز، وتقويمُ الجهازِ قد يُزامَن
      // إلى حسابٍ ويُشارَك في عمل. فيُقال ذلك قبل الضغطِ لا بعدَه — وقد كُتِب
      // في الملفِّ نفسِه `CLASS:PRIVATE` و`TRANSP:TRANSPARENT`، وهما وصفٌ
      // يعمل به ما يعمل ويُهمِله ما يُهمِل، فلا يُوعَد بهما وعدَ قُفل.
      el('span.fine', { style: { textAlign: 'start' } },
        'والحدثُ يُكتَب في تقويمك أنت — فإن كان تقويمك مُزامَناً أو مشاركاً '
        + 'مع غيرك ظهر عندهم. وهو مكتوبٌ «خاصّاً» و«لا يشغل وقتك»، وذلك ما '
        + 'يملكه ملفُّ التقويم، لا أكثر.'),
      /*
