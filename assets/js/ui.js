...
// `owner` وحدةٌ بلا استيراداتٍ البتّة، فلا دورةَ في الاستيراد. ولا يُستعمَل
// منها ههنا إلا `isOwner` لحصرِ شارةِ المطوّرِ في صاحبِ التطبيق.
 ...
 * فيُرسَل البلاغُ برقمِ السؤالِ وعلمِه وموضعِه وصفحتِه — كي يُعرَف السؤالُ من
 * رقمِه بلا بحثٍ، ويُقابَل على صفحتِه مباشرةً. ولا يُرسَل من التطبيق شيءٌ عن
 * الطالب.
 *
 * ── «الباب» كان يُقال على ما ليس بباب ───────────────────────────────────
 *
 * كان يُكتَب `الباب: ${'${q.topic}'}` — و`topic` اسمٌ مجرَّدٌ نحو «الغصب»، فيخرج
 * البلاغُ يقول «الباب: الغصب». وهو في دليل الطالب **كتابٌ** لا باب: ص١٦٩
 * تبدأ بـ«كتاب الغصب». والفرقُ بين الكتابِ والبابِ والفصلِ معروفٌ عند من
 * يقرأ في كتب الفقه — وأوّلُ من أبلغَ عن هذا أبلغَ عنه.
 *
 * وليست حالةً واحدة: في بنكِ الفقهِ ألفٌ ومائةٌ وثلاثةٌ وسبعون سؤالاً مرجعُها
 * «كتاب …» وأربعُ مئةٍ وستةٌ وعشرون مرجعُها «باب …».
 *
 * والصوابُ حاضرٌ في البيانات ولا يُقرَأ: لكلِّ سؤالٍ `reference` فيه عنوانُ
 * موضعِه كما هو في الكتاب («كتاب الغصب»، «كتاب الصلاة — سنن الأفعال»). فلا
 * يُخترَع له وصفٌ، ويُنقَل عنوانُه.
 */
export function reportLink(q) {
  const where = q.reference || q.topic || '';
  const href = waLink(
    `بلاغُ خطأ في سؤال\n\nرقم السؤال: ${q.id}\nالعلم: ${q.subject || '—'}`
    + `${where ? `\nالموضع: ${where}` : ''}${q.bookPage ? `\nالصفحة: ${q.bookPage}` : ''}`
    + `\n\nالسؤال: ${q.question}\n\nالخطأ الذي وجدتُه: `);
  if (!href) return null;
 ...
 *
 * ── والاسمُ يُضغَط فيفتح واتساب ────────────────────────────────────────────
 *
 * وذلك أنّ الطالبَ يقع على الخطأ أو يَعرِض له السؤالُ في أيِّ شاشةٍ كان،
 * وبابُ المراسلةِ الوحيدُ كان في «حسابي» — شاشةٌ لا يدخلها إلا قاصدٌ لها.
 * والاسمُ حاضرٌ في ذيلِ كلِّ شاشة، فهو أقربُ بابٍ إلى يده.
 *
 * والرابطُ على **الاسمِ وحدَه** لا على الجملةِ كلِّها: الجملةُ خبرٌ، والاسمُ
 * هو الجهةُ التي تُراسَل — فلا يُوسَّع المضغوطُ على ما ليس مقصوداً بالضغط.
 *
 * ── وموضعُ اللمسِ أربعةٌ وأربعون ──────────────────────────────────────────
 *
 * والحرفُ ١١٫٥ بكسلاً فارتفاعُ سطرِه نحوُ إحدى وعشرين — ولو تُرِك كذلك لكان
 * رابطاً لا تُصيبه إصبعٌ، وأولى الناسِ بهذا التطبيقِ كبارُ السنّ. فيُوسَّع
 * بـ`inline-flex` وارتفاعٍ أدنى، ويُنقَص من حشوِ `.credit` بقدرِ ما زاد —
 * فلا يتبدّل موضعُ السطرِ على الشاشةِ ولا ارتفاعُ ذيلِها (٤٩ ← ٤٨ بكسلاً).
 *
 * ولا يُوضَع عليه `.tap-inline` — تلك لِما لا يقوم موضعُه إلا صغيراً، لا
 * لتسكيتِ الفحص. وههنا يقوم كبيراً بلا ثمن.
 *
 * وإن أُفرِغ `CONTACT.whatsapp` عاد السطرُ نصّاً كما كان، فلا رابطَ إلى لا أحد.
 */
export const credit = () => {
  const href = waLink('السلام عليكم، عندي ملاحظةٌ على منصة الاستعداد:\n\n');
  if (!href) return el('p.credit', 'تم تطوير التطبيق بواسطة بدر المسلم');
  return el('p.credit', [
    ...
    el('a.credit-name', {
      ...
      target: '_blank',
      rel: 'noopener',
      'aria-label': 'راسِل بدر المسلم على واتساب',
    }, 'بدر المسلم'),
  ]);
};
 ...
 * تُفعَّل بـ`?dev=1` وتُطفَأ بـ`?dev=0`.
 *
 * ── و«لا يراها الطالب» صارت صحيحةً بعد أن كانت دعوى ────────────────────
 *
 * كان هذا التعليقُ يقول «ولا يراها الطالب البتّة»، ومثلُه في `app.js` — وهما
 * يكذبان: `?dev=1` مَسلَكٌ مفتوحٌ لكلِّ أحد، فمن زادَه على الرابطِ رأى شارةَ
 * «لم يُقابَل» على **٣٥٣ سؤالاً**، وهي حالٌ داخليّةٌ يُفسِّرها صاحبُ التطبيقِ
 * ولا يُفسِّرها الطالب: يحسبها حكماً على السؤالِ بالخطأ وهي خبرٌ عن مرحلةِ
 * التوثيق.
 *
 * فقُيِّدت بصفةِ المالكِ كما كان يُدَّعى. و`owner.js` بلا استيرادٍ أصلاً، فلا
 * دورةَ بين الوحدتَين.
 */
export function devBadge(q) {
  if (!devMode() || !owner.isOwner() || q.bookVerified) return null;
